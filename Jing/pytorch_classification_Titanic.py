...
BATCH_SIZE = 99                 #每一批数据batchsize 99 个
...
def show_batch():
    for epoch in range(15):   # train entire dataset 15 times
        for step, (batch_x, batch_y) in enumerate(loader):  # for each training step
            ...
            prediction = torch.max(F.softmax(out), 1)[1] #softmax函数显示了输出的概率，torchmax取概率最大的那一列的列（即0和1之间选一个）
            pred_y = prediction.data.numpy().squeeze()
            target_y = batch_y.data.numpy()
            accuracy = sum(pred_y == target_y)/99.  # 预测中有多少和真实值一样，计算准确率
            print(epoch)
            print(step)
            print(accuracy)
            print(target_y)
...
passergerIDtest1=passergerIDtest1.squeeze()#passengerId1是后来用来合并为dataframe用的list，但是print(passergerIDtest)不可以写print(passergerIDtest1)，要不然就是会带入神经网络里面去执行运算
out1 = net(x1)
prediction1 = torch.max(F.softmax(out1), 1)[1] 
pred_y1 = prediction1.data.numpy().squeeze()
print(passergerIDtest)                      #print(passergerIDtest)如果写print(passergerIDtest1)，预测时也显示出来了，导致混乱
print(pred_y1)

#这里面是读取了gender_submission.csv里面的一个估计值，顺便比一下准确率
...
#合成新的dataframe，并打印在test_pred.csv文件里面
